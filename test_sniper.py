...
def test_history_api():
    """保存機能（履歴API）のテスト"""
    print("=== 履歴API（/history, /history/{id}, /history/{id} DELETE）テスト ===")
    # 履歴一覧取得
    res = requests.get("http://localhost:8000/history?limit=5")
    print(f"/history ステータス: {res.status_code}")
    history = res.json()
    print(f"履歴件数: {len(history)}")
    if history:
        first_id = history[0]['id']
        # 詳細取得
        detail = requests.get(f"http://localhost:8000/history/{first_id}")
        print(f"/history/{first_id} ステータス: {detail.status_code}")
        print(f"詳細: {json.dumps(detail.json(), indent=2, ensure_ascii=False)}")
        # 削除
        delete = requests.delete(f"http://localhost:8000/history/{first_id}")
        print(f"/history/{first_id} DELETE ステータス: {delete.status_code}")
        print(f"削除レスポンス: {delete.json()}")
    print()

def test_statistics_api():
    """統計情報APIのテスト"""
    print("=== 統計API（/statistics）テスト ===")
    res = requests.get("http://localhost:8000/statistics")
    print(f"/statistics ステータス: {res.status_code}")
